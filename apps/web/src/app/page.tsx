...
function HomePageContent() {
  ...
  const localBackendRef = useRef<Window["desktop"]["engageoscribeBackend"] | null>(null)
  ...
  useEffect(() => {
    const loadApiKeys = async () => {
      try {
        ...
        setHasGeminiApiKey(mixedAuthStatus.hasGeminiKeyConfigured)
        setMixedAuthSource(mixedAuthStatus.source)
        if (mixedAuthStatus.hasGeminiKeyConfigured) {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return
    const backend = window.desktop?.engageoscribeBackend
  ...
  useEffect(() => {
    setShowMixedKeyPrompt(mixedAuthStatusLoaded && processingMode === "mixed" && !hasGeminiApiKey)
  }, [mixedAuthStatusLoaded, processingMode, hasGeminiApiKey])

  useEffect(() => {
    if (typeof window === "undefined") return
    if (window.__engageoscribePermissionsPrimed) return
    if (permissionCheckInProgressRef.current) return
    
    window.__engageoscribePermissionsPrimed = true
  ...
  const handleProcessingModeChange = useCallback(async (mode: ProcessingMode) => {
    ...
    return true
  }, [ensureLocalRuntimeReady, hasGeminiApiKey])

  const handleSaveGeminiApiKey = useCallback(async (value: string) => {
    const trimmed = value.trim()
    await setApiKeys({ geminiApiKey: trimmed })
    const status = await getMixedModeAuthStatus()
    setHasGeminiApiKey(status.hasGeminiKeyConfigured)
  ...
  const handleStartRecording = async (data: {
    ...
    try {
      ...
      if (!useLocalBackend && !hasGeminiApiKey) {
        setShowMixedKeyPrompt(true)
        setShowSettingsDialog(true)
        return
      }
      ...
      if (useLocalBackend && localBackendRef.current) {
        const sessionName = `Engageo Scribe ${encounter.id}`
  ...
  return (
    <>
      ...
      <SettingsDialog
        ...
        onGeminiApiKeyChange={setGeminiApiKeyInput}
      ...
      {showMixedKeyPrompt && processingMode === "mixed" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-border bg-background p-6 shadow-2xl">
            <h3 className="text-lg font-semibold text-foreground">Gemini Key Required for Mixed Mode</h3>
            <p className="mt-2 text-sm text-muted-foreground">
              Mixed mode uses Gemini for note generation. Add your Gemini key in Settings, or switch to local-only mode.
            </p>
      ...
      <div className="flex h-screen w-screen overflow-hidden bg-background">
        <div className="flex h-full w-72 shrink-0 flex-col overflow-hidden border-r border-sidebar-border bg-sidebar">
          <div className="stripe-bar shrink-0" aria-hidden="true">
            <span />
            <span />
            <span />
          </div>
