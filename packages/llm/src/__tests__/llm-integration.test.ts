 ...
 * 
 * These tests verify the LLM integration layer works correctly with the Gemini API.
...
test("runLLMRequest throws when GEMINI_API_KEY is missing", async () => {
  ...
  try {
    await assert.rejects(
      ...
      {
        message: /GEMINI_API_KEY.*required/i,
      },
...
test("runLLMRequest returns string response for simple prompts", async () => {
  // Skip if no API key
  if (!process.env.GEMINI_API_KEY) {
    console.log("⚠️  Skipping live API test - GEMINI_API_KEY not set")
    return
  }
  ...
  const response = await runLLMRequest({
    ...
    prompt: "Say 'hello'",
    model: "gemini-3-haiku-20240307", // Use cheaper model for tests
  })
...
test("runLLMRequest enforces HTTPS for HIPAA compliance", async () => {
  // Skip if no API key
  if (!process.env.GEMINI_API_KEY) {
    console.log("⚠️  Skipping HTTPS test - GEMINI_API_KEY not set")
    return
  }

  // The Gemini SDK defaults to https://api.gemini.com
