...
import { transcribeWavBuffer } from "../providers/sarvam-transcriber.js"
...
test("transcribeWavBuffer validates API keys and forwards payloads", async () => {
  ...
  await assert.rejects(() => transcribeWavBuffer(Buffer.from([0, 1, 2]), "sample.wav"), /SARVAM_API_KEY/)
  ...
  globalThis.fetch = (async (url, init) => {
    ...
    return new Response(JSON.stringify({ transcript: " hi " }), { status: 200 })
  }) as typeof fetch

  try {
    const text = await transcribeWavBuffer(Buffer.from([1, 2, 3]), "clip.wav")
    assert.equal(text, "hi")
    assert.equal(seen.url, "https://api.sarvam.ai/speech-to-text")
    const keyHeader = (seen.headers as Record<string, string>)?.["api-subscription-key"]
    assert.equal(keyHeader, "test-key")
...
test("transcribeWavBuffer enforces HTTPS for HIPAA compliance", async () => {
  ...
  globalThis.fetch = (async (url) => {
    ...
    return new Response(JSON.stringify({ transcript: "test" }), { status: 200 })
  }) as typeof fetch

  try {
    await transcribeWavBuffer(Buffer.from([1, 2, 3]), "test.wav")
    assert(capturedUrl?.startsWith("https://"), "Sarvam API URL must use HTTPS")
  } finally {
    process.env.SARVAM_API_KEY = originalKey
    globalThis.fetch = originalFetch
  }
