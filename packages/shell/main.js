...
const isE2ESmoke = process.env.ENGAGEOSCRIBE_E2E_SMOKE === '1';

// Set app name (for development mode and dock)
if (app) {
  app.setName('Engageo Scribe');
}
...
const boot = async () => {
  ...
  if (checkIfRunningFromDMG()) {
    const response = dialog.showMessageBoxSync({
      ...
      message: 'Please install Engageo Scribe to your Applications folder',
      detail: 'Running directly from the disk image may cause startup issues. Please drag Engageo Scribe to your Applications folder and run it from there.\n\nWould you like to continue anyway?',
  ...
  registerEngageoScribeIpcHandlers(mainWindow);
  ...
  if (isE2ESmoke) {
    ...
    if (!probe.success) {
      console.error('ENGAGEOSCRIBE_E2E_SMOKE_FAIL', JSON.stringify(probe));
      app.exit(1);
      return;
    }
    console.log('ENGAGEOSCRIBE_E2E_SMOKE_PASS');
...
if (!gotTheLock) {
  ...
  console.log('Another instance of Engageo Scribe is already running. Exiting.');
  app.quit();
  process.exit(0);
} else {
