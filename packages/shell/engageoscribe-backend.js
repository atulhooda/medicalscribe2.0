...
// Backend executable path - use bundled EngageoScribe backend
function getBackendPath() {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, 'engageoscribe-backend', 'engageoscribe-backend');
  }
  return path.join(process.cwd(), 'local-only', 'engageoscribe-backend', 'dist', 'engageoscribe-backend', 'engageoscribe-backend');
}

function getBackendCwd() {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, 'engageoscribe-backend');
  }
  return path.join(process.cwd(), 'local-only', 'engageoscribe-backend', 'dist', 'engageoscribe-backend');
}
...
function resolveBackendCommand(args = []) {
  ...
  const backendProjectRoot = path.join(process.cwd(), 'local-only', 'engageoscribe-backend');
...
function getBackendDataDir() {
  if (process.platform === 'darwin') {
    return path.join(os.homedir(), 'Library', 'Application Support', 'engageoscribe-backend');
  }
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming');
    return path.join(appData, 'engageoscribe-backend');
  }
  const xdgData = process.env.XDG_DATA_HOME || path.join(os.homedir(), '.local', 'share');
  return path.join(xdgData, 'engageoscribe-backend');
}
...
function runPythonScript(mainWindow, script, args = [], silent = false) {
  return new Promise((resolve, reject) => {
    ...
    if (!silent) {
      sendDebugLog(mainWindow, `$ engageoscribe-backend ${args.join(' ')}`);
      if (backend.mode === 'python-fallback') {
...
async function ensureWhisperService(mainWindow) {
  ...
  if (!backend.command) {
    return {
      ...
      userMessage: 'Whisper service command is unavailable. Reinstall EngageoScribe and retry.',
    };
  }
...
function truncateForPrompt(value, maxChars) {
  ...
  return `${text.slice(0, maxChars)}\n\n[Truncated by EngageoScribe for transport safety]`;
}
...
function buildOpenClawInstruction(payload) {
  ...
  return [
    'You are receiving a structured handoff from EngageoScribe.',
...
async function dispatchOpenClawChatTurn(params) {
  ...
  const sessionId = requestedSessionId || defaultSessionId || `engageoscribe-${encounterId || 'session'}`;
...
function registerEngageoScribeIpcHandlers(mainWindow) {
  ...
  ipcMain.handle('start-recording-ui', async (_, sessionName, noteType = 'history_physical') => {
    try {
      ...
      sendDebugLog(mainWindow, '$ engageoscribe-backend record 7200');
  ...
  ipcMain.handle('setup-ollama-and-model', async (_event, requestedModel) => {
    try {
      ...
      sendDebugLog(mainWindow, `$ engageoscribe-backend pull-model ${selectedModel}`);
  ...
  ipcMain.handle('get-ai-prompts', async () => {
    try {
      const summarizerPath = path.join(process.cwd(), 'local-only', 'engageoscribe-backend', 'src', 'summarizer.py');
    ...
    try {
      ...
      if (!whisperStatus?.success) {
        ...
        const userMessage = reason === 'STARTING'
          ...
          : 'Whisper service is not healthy. Retry in a few seconds or restart EngageoScribe.';
  ...
  ipcMain.handle('ensure-local-runtime-ready', async () => {
    try {
      ...
      if (!whisperStatus?.success) {
        ...
        const userMessage = reason === 'STARTING'
          ? 'Whisper is still initializing in the background. Retry in a few seconds.'
          : 'Whisper service is not healthy. Retry setup or restart EngageoScribe.';
...
async function checkForUpdates() {
  return new Promise((resolve) => {
    const options = {
      ...
      path: '/repos/engageo/EngageoScribe/releases/latest',
      method: 'GET',
      headers: { 'User-Agent': 'EngageoScribe-Updater' },
    };
...
async function runBackendHealthProbe() {
  try {
    ...
    if (process.env.ENGAGEOSCRIBE_E2E_STUB_PIPELINE === '1') {
...
module.exports = {
  registerEngageoScribeIpcHandlers,
