...
class OllamaSummarizer:
    def __init__(self, model_name: Optional[str] = None):
        ...
        self._chat_options = {
            "temperature": float(os.getenv("ENGAGEOSCRIBE_OLLAMA_TEMPERATURE", "0.1")),
            "top_p": float(os.getenv("ENGAGEOSCRIBE_OLLAMA_TOP_P", "0.9")),
            "num_predict": int(os.getenv("ENGAGEOSCRIBE_OLLAMA_NUM_PREDICT", "260")),
        }
        self._keep_alive = os.getenv("ENGAGEOSCRIBE_OLLAMA_KEEP_ALIVE", "30m")
    ...
    def _warmup_model(self) -> None:
        ...
        if os.getenv("ENGAGEOSCRIBE_OLLAMA_WARMUP", "0").lower() not in {"1", "true", "yes", "on"}:
            return
        try:
            t0 = time.perf_counter()
            self.client.chat(
